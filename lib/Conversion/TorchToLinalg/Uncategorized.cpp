  ...
  matchAndRewrite(AtenLinalgCholeskyExOp op, OpAdaptor adaptor,
    ...
    auto generateLLoop = rewriter.create<scf::ForOp>(
        ...
        [&](OpBuilder &b, Location loc, /*index*/ Value colIndex,
          ...
          Value col0 = rewriter.create<tensor::EmptyOp>(loc, colDims, elemTy);

          Value col1 = rewriter
                           .create<linalg::FillOp>(loc, ValueRange{cstZero},
                                                   ValueRange{col0})
                           .getResult(0);
          SmallVector<OpFoldResult> iiOffsets = {colIndex, colIndex};
          Value aii = rewriter.create<tensor::ExtractOp>(loc, elemTy, input);
